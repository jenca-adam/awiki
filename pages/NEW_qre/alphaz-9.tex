...
The $\alpha-z$-R\'enyi divergences were introduced in \cite{audenaert2015alpha}
as a two parameter family of quantum generalizations of  the classical R\'enyi
divergences. For a pair of density operators (or more generally positive
operators) $\rho$ and $\sigma$ on a finite dimensional Hilbert space, the divergence
$D_{\alpha,z}(\rho\|\sigma)$ is defined by 
\[
D_{\alpha,z}(\rho\|\sigma)=\frac{1}{\alpha-1}\log\frac{\Tr(\rho^{\frac{\alpha}{2z}}\sigma^{\frac{1-\alpha}{z}}\rho^{\frac{\alpha}{2z}})}{\Tr\rho},
\]
where $0<\alpha\ne 1$ and $z>0$. This family includes the quantum relative entropy in the
...
quasi-entropy \cite{petz1985quasi,petz1986quasi} are obtained for $z=1$,  whereas $z=\alpha$ gives the sandwiched
R\'enyi divergences introduced in \cite{mullerlennert2013onquantum, wilde2014strong}. These two
quantities have important applications in...

A fundamental property of any information measure is the data processing inequality (DPI), that
is, such a quantity must be nonincreasing under quantum channels, which are defined as completely
positive trace preserving maps. For the R\'enyi divergences, this means that we require
\[
...
methods and, as later observed in \cite{mullerhermes2017monotonicity}, it shows that DPI holds also for maps that are trace
preserving and positive but not necessarily completely positive. Remarkably, by taking the limit
$\alpha\searrow 1$, this proves this property also for the relative entropy. 
...
\max\{\alpha,\alpha-1\}\le z\le \alpha.
\]
...
algebras form a suitable framework for such extensions, since their are general enough to
...
already extended to this setting, notably the Araki relative entropy
\cite{araki1976relative}. The
quasi-entropies were first introduced for states of von Neumann algebras
\cite{petz1985quasi} and
studied in detail in \cite{hiai2018quantum}. The definition of  sandwiched R\'enyi divergences  in
this setting, based on noncommutative $L^p$-spaces,  was
introduced independently in \cite{berta2018renyi}  using Araki-Masuda $L^p$-spaces and in  \cite{jencova2018renyi,
jencova2021renyi} using Kosaki $L^p$-spaces. A number of properties of the divergences
including DPI was proved in these works and it was shown in \cite{jencova2018renyi} that
the two definitions are indeed equivalent. See \cite{hiai2021quantum} for an overview of both the Petz-type and
sandwiched R\'enyi divergences in the von Neumann algebra setting.

Sufficiency quantum channels with respect to a set of states was introduced by Petz
\cite{petz1986sufficient,petz1988sufficiency}. This definition was inspired by sufficient statistics 
for classical statistical models. The quantum counterpart in Petz's definition is that all
...
as for the transition probability (which is basically the Petz-type R\'enyi divergence for
...
for the standard R\'enyi divergence was proved in \cite{hiai2017different} in the interval $(0,1)\cup(0,2)$
and in \cite{jencova2017preservation, jencova2018renyi, jencova2021renyi} for the sandwiched version with $\alpha\in
(1/2,1)\cup(1,\infty)$. See also \cite{hiai2021quantum} for a summary of these result for
von Neumann algebras.  



The $\alpha-z$-R\'enyi divergences $D_{\alpha,z}$ in von Neumann algebras were introduced in
\cite{kato2023aremark} and a number of their properties was subsequently proved
in \cite{kato2023onrenyi}. In particular, most of the important properties including the
DPI (with respect to normal positive unital maps), monotonicity in the parameter $z$ and a
variational expression were proved in the case $0<\alpha<1$. In the type I case,
...
We first finish the proof of the variational expression for $\alpha>1$ and prove the DPI,
in the same bounds as in the finite dimensional case. Similarly to Zhang in
\cite{zhang2020fromwyd}, we apply the variational expressions to so this, but, not having the
techniques of matrix analysis at our disposal, we use their relation to the sandwiched
R\'enyi divergences for the proof. This allows us to show the DPI with respect to normal
positive unital maps, similarly as in the case $0<\alpha<1$ in \cite{kato2023onrenyi} and
for the sandwiched R\'enyi divergences in \cite{jencova2018renyi,jencova2021renyi}. As as
a consequence, we prove martingale convergence theorems for $D_{\alpha,z}$. 
We further show that for the values of $(\alpha,z)$ inside the DPI bounds, equality in the
DPI implies sufficiency of the channel, where by channel we here mean a normal 2-positive
unital map. The proofs again use the variational expressions and their relation to the sandwiched
R\'enyi divergences, together with the properties of conditional expectations. 

We then turn to monotonicity in the parameters $z$ and $\alpha$. We first show that in the
...
on the interval $[\alpha/2,\infty)$ using complex interpolation method. In addition, we
also  prove monotonicity in the parameter $\alpha$, which seems to be new even in the finite
dimensional case,  and the limits $\alpha\searrow 1$ and $\alpha\nearrow 1$.
...
$L^1(\Me)^{(1)}$, where $\Tr_j$ is the $\Tr$-functional on
\textcolor{orange}{$L^1(\Me)^{(j)}$}. (For more details on the isomorphism
...
\end{align*}
\textcolor{orange}{Since $p,r\ge 1$, both terms in brackets in the last expression above are nonnegative.
...
a)\|_p=Q_{\alpha,z}(\psi\|\varphi)$ by \eqref{eq:infimum}, this proves the statement.} The proof in the case $r>1$ is similar. 
\end{proof}
...
\cite{junge2003noncommutative}, {\color{orange} see also Appendix \ref{app:lp}.} 
...
{\color{orange} Since $\psi$ is assumed faithful, we may construct the Haagerup
$L^p$-spaces with respect
...
\begin{theorem}\label{thm:suffge1} \textcolor{orange}{Assume that $1<\alpha<z+1$}.
...
\]
Similarly as in the proof of Theorem \ref{thm:suffle1} \textcolor{orange}{(note that we
...
{\color{orange} Using again the
extensions of $\cE$ and their properties in  Appendix \ref{app:lp},}
\[
...
integrable on $(0,\infty)$, the Lebesgue convergence theorem yields \eqref{F-4.5}.
...
\]
one has, similarly to \eqref{F-4.8},
...
\]
for all $s>0$. Therefore, by \eqref{F-4.8} and \eqref{F-4.9} the monotone convergence theorem yields
\eqref{F-4.7}.
\end{proof}
...
Then by \eqref{F-4.11}, $F(\alpha)=\log Q_{\alpha,\infty}(\psi\|\ffi)$ for all $\alpha\in(0,\infty)\setminus\{1\}$,
and we compute
\begin{align*}
...
=D_1(\psi\|\ffi).
\]
Hence by \eqref{F-4.10} the assertion holds when $h_\psi,h_\ffi\in\Me^{++}$. Below we extend this to
...
(h_\ffi^{-1})^{\alpha-1\over2z}\Bigr)^z\,ds
=\lim_{n\to\infty}Q_{\alpha,z}(\psi_n\|\ffi). \label{F-4.12}
\end{align}
...
$h_{\psi_n}^{\alpha/z}\nearrow h_\psi^{\alpha/z}$ in the measure topology, one can argue as in \eqref{F-4.12}
...
\]
Hence \eqref{F-4.13} follows since it holds for $D_{\alpha,1}$ by
\cite[Proposition 5.3(3)]{hiai2018quantum}, as stated in \eqref{F-4.1}.
...
\]
Hence \eqref{F-4.14} follows since it holds for $D_{\alpha,\alpha}$ by
\cite[Proposition 3.8(ii)]{jencova2018renyi}, as stated in \eqref{F-4.2}.
\end{proof}}
...
case \cite{lin2015investigating,mosonyi2023somecontinuity}. \textcolor{orange}{By  Theorem
...
we will extend the  limits given in \eqref{F-4.13} and \eqref{F-4.14} in Sec.~5.3.}
...
For each $a\in\widetilde\cR$ satisfying \eqref{F-5.1} we define (see \cite{fack1986generalized})
\[
...
satisfies \eqref{F-5.1}. Also, note that if $a,b\in\widetilde\cR$ satisfy \eqref{F-5.1}, then $|a|^p$ $(p>0$)
and $ab$ satisfy \eqref{F-5.1} too, as it is clear since $\mu_t(ab)\le\|a\|\mu_t(b)$ for $a\in\cR$,
$\mu_t(|a|^p)=\mu_t(a)^p$, and $\mu_t(ab)\le\mu_{t/2}(a)\mu_{t/2}(b)$; see
\cite[Lemma 2.5]{fack1986generalized}.

\begin{prop}\label{P-5.2}
Let $a_j,b_j\in\widetilde\cR^+$, $j=1,2$, satisfying \eqref{F-5.1} and assume that $a_1a_2=a_2a_1$
...
\end{align*}
Letting $k\to\infty$ gives \eqref{F-5.2}. When $\theta=1/2$, \eqref{F-5.2}  is rewritten as \eqref{F-5.3}.
\end{proof}
...
$\eta:={z-\alpha\over2z-1}=(1-\theta)\eta_1+\theta\eta_2$. From \eqref{F-5.7} and the reiteration
theorem, note that
...
$\eta:=1-\alpha=(1-\theta)\eta_1+\theta\eta_2$. With $f$ given in \eqref{F-5.10}
...
%From \eqref{F-5.11} and \eqref{F-5.9} (for $\eta,\eta_1,\eta_2$) we have \eqref{F-5.8}, so that (1) has been
...
%$f$ given in \eqref{F-5.10} define $f_1(w):=f((1-w)\eta_1+w\eta_2)$, which belongs to the set
...
based on the complex interpolation {\color{orange} in a similar way as discussed in Remark \ref{remark:2nd-proof}.}
...
\eqref{F-5.9} holds in the present situation. For this, we may assume that
$Q_{\alpha_j,z}(\psi\|\ffi)<\infty$, $j=1,2$. Then we can use \eqref{F-5.8} similarly to the discussion
in Remark \ref{remark:2nd-proof} with $h_\psi$ instead of $h_0$. Hence we have \eqref{F-5.9},
and (1) follows.
...
below and \eqref{F-4.1} for $D_{\alpha,1}$. On the other hand, for the case $z\in[1,\infty)$ the result
follows from Proposition \ref{P-4.6}(i) as \eqref{F-4.13} does from Proposition \ref{P-4.5}(i).
\end{proof}
...
$\ffi$ is faithful. We utilize the function $f$ on $S$ given in \eqref{F-4.15}, whose values are in $L^{2z}_L$
...
&=\psi(\1)^{-{2z-1\over2z}}{\alpha\over2z}\,D(\psi\|\ffi), \label{F-5.12}
\end{align}
where we have used \eqref{F-C.5} for the third equality and \cite[Theorem 5.7]{ohya1993quantum}
for the last equality. Since $\psi(\1)=\|f(1/\alpha)\|_{L^{2z}_L}^{2z}$, it follows from \eqref{F-5.11} and
\eqref{F-5.12} that
\begin{align*}
D_{\alpha',z}(\psi\|\ffi)
...
$\tau\circ\theta_s=e^{-s}\tau$, $s\in\bR$; see \cite[Chap.~X]{takesaki2003theory2} (also
...
{\color{orange}We will next recall the extensions of a conditional expectations to the
Haagerup $L^p$-spaces, obtained in \cite{junge2003noncommutative}. 
Let $\Ne\subseteq \Me$ be a subalgebra such that
$\sigma^\omega(\Ne)\subseteq \Ne$. Equivalently, there is a faithful normal conditional
...
the first equality in \eqref{F-4.16}. By Theorem \ref{T-B.1} we can apply the martingale convergence
in Theorem \ref{thm:martingale} to obtain the latter equality in \eqref{F-4.16} with increasing convergence.
The assertion of $D_1$ in \eqref{F-4.17} is included in \cite[Proposition 2.2]{fawzi2023asymptotic},
...
\newblock \emph{Journal of Mathematical Physics}, 56\penalty0 (2), 2015.
...
\newblock Sandwiched {R{\'e}nyi} divergence satisfies data processing
  inequality.
\newblock \emph{Journal of Mathematical Physics}, 54\penalty0 (12), 2013.
...
\newblock \emph{Journal of Physics A: Mathematical and Theoretical},
  51\penalty0 (48):\penalty0 483001, 2018.
...
\newblock Different quantum f-divergences and the reversibility of quantum
  operations.
\newblock \emph{Reviews in Mathematical Physics}, 29\penalty0 (07):\penalty0
  1750023, 2017.
...
\newblock \emph{Journal of Physics A}, 50:\penalty0 085303, 2017.
...
M.~M. M{\" u}ller Lennert, F.~Dupuis, O.~Szehr, S.~Fehr, and M.~Tomamichel.
\newblock {On quantum R\'enyi entropies: a new generalization and some
  properties}.
\newblock \emph{{J. Math. Phys.}}, {54}:\penalty0 {122203}, {2013}.
