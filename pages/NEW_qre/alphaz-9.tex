...
%The classical R\'enyi divergences form a one-parameter family of dissimilarity measures on
%pairs of probability vectors. These quantities and derived information measures are fundamental in information theory. 
%In an axiomatic approach, it was proved by R\'enyi \cite{renyi} that they form the unique
%family of such measures satisfying certain properties. For quantum states,
%however, such uniqueness is no longer true, there are many different possible
%noncommutative extensions of the R\'enyi divergences with resonable properties and at
%least two of them have already established their importance through their operational
%significance.  These are the Petz-type or standard R\'enyi divergences, based on Petz's quasi-entropies
%\cite{petz,petz} and the sandwiched R\'enyi divergences \cite{...}. ...
%
...
quasi-entropy \cite{petz} are obtained for $z=1$,  whereas $z=\alpha$ gives the sandwiched
R\'enyi divergences introduced in \cite{,}. These two
...
leads to a restriction on the values of the parameters. For the Petz-type divergences,
 the convexity theorem of Ando \cite{ando} shows that  $\alpha$ is restricted to the interval
$(0,1)\cup (1,2]$. The DPI in the sandwiched case holds for $\alpha\in
[1/2,1)\cup(1,\infty)$, as was proved in  \cite{frank}  and independently in
\cite{beigi}. The proof in \cite{beigi} was given only for
the case $\alpha>1$, but is interesting for us, because it utilizes complex interpolation
methods and, as later observed in \cite{mh}, it shows that DPI holds also for maps that are trace
...
already extended to this setting, notably the Araki relative entropy \cite{araki}. The
quasi-entropies were first introduced for states of von Neumann algebras \cite{petz} and
studied in detail in \cite{hiai}. The definition of  sandwiched R\'enyi divergences  in
...
divergences and other quantities satisfying the DPI \cite{}. In particular, this property
for the standard R\'enyi divergence was proved in \cite{} in the interval $(0,1)\cup(0,2)$
and in \cite{jencova2018renyi, jencova2021renyi} for sandwiched version with $\alpha\in
(1/2,1)\cup(1,\infty)$. See also \cite{hiai2021quantum} for a summary.  Various  approximate versions were studied as
well, in both the finitine dimensional \cite{} and von Neumann algebra setting \cite{}. 



The $\alpha-z$-R\'enyi divergences $D_{\alpha,z}$ in von Neumann algebras were introduced in
\cite{kato2023aremark} and a number of their important properties was subsequently proved
...
variational expression were proved in the case $0<\alpha<1$. 
...
We then turn to monotonicity in the parameters $z$ and $\alpha$.



especially the variational expression, DPI and monotonicity in $z$ for $\alpha>1$. 
