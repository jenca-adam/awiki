...
\cite{mosonyi2023thestrong}. {\color{orange} In the von Neumann algebra setting, an alternative definition for $\alpha\in
[0,1]$ and $z\ge 1/2$ was given in \cite{furuya2023monotonic}, where the DPI with respect
to completely positive normal unital maps was also proved for $z>1$}{\color{magenta} [I
did not yet confirm that their definition is the same as ours, but I think a citation
...
algebra case. {\color{orange}The main result of this section is the proof of the following
variational expressions}{\color{magenta}[I think this is better, the formulation before
was ''The main theorem ... is the following variational expressions'', this seems
gramatically strange to me.]}, which will
...
\end{align}
{\color{orange} Moreover, the equality \eqref{F-2.2} holds for all $\alpha>1$ and $z>0$ such that
...
{\color{orange} $y:=h_\psi^{\frac{1}{2z}}b\in L^{2z}(\Me)$}. By Lemma \ref{lemma:renyi_2z},
$Q_{\alpha,z}(\psi\|\varphi)=\|y\|_{2z}^{2z}<\infty$. This shows the {\color{orange} last} assertion too.
...
%bounds shown in Theorem \ref{thm:dpi} below. However,
...
Assume that $\psi\sim\ffi$ {\color{orange}and let $0<\alpha<1$,
$\max\{\alpha,1-\alpha\}\le z$}{\color{magenta}[We need this assumption to use Kato's
proof, we need to put it here since it is no longer assumed in Theorem 2.4]}. Then the infimum in \eqref{F-2.1} of Theorem \ref{thm:variational}(i) is
...
\]
Furthermore, {\color{orange} since $f(\alpha'/\alpha)\in L^{2z}_L$, we have
...
\lim_{t\to0}{\color{orange}{1\over it}}\Tr\bigl[h_\psi\bigl(h_{\psi_0}^{it}h_\ffi^{-it}-\1\bigr)\bigr] \nonumber\\
...
\bibitem[Furuya et~al.(2023)Furuya, Lashkari, and Ouseph]{furuya2023monotonic}
K.~Furuya, N.~Lashkari, and S.~Ouseph.
\newblock Monotonic multi-state quantum f-divergences.
\newblock \emph{Journal of Mathematical Physics}, 64\penalty0 (4), 2023.
\newblock \doi{10.1063/5.0125505}.
...
{\color{red}
\bibitem[Halmos and Savage(1949)]{halmos1949application}
P.~R.~Halmos and L.~J.~Savage.
\newblock Application of the Radon-Nikodym theorem to the theory of sufficient statistics.
\newblock \emph{Annals of Mathematical Statistics}, 20\penalty0 (2): \penalty 0
225--241, 1949.
\newblock \doi{10.1214/aoms/1177730032}.
}
...
{\color{red}
\bibitem[Kullback and Leibler(1951)]{kullback1951oninformation}
