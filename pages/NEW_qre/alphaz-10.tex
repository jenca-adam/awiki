...
%[Since ''data-processing inequality'' is also a kind of monotonicity, it might be better
%to add ''in $\alpha,z$'', that makes the meaning of monotonicity properties clearer.]
%}

%$\alpha$-$z$-R\'enyi divergences in von Neumann algebras via non-commutative $L^p$-spaces
%}}
...
\begin{abstract}
This paper is aimed at following up S.~Kato's paper ``On $\alpha$-$z$-R\'enyi divergence in the
von Neumann algebra setting''. We finish the proofs of the variational expression and of the data processing
inequality (DPI) for the $\alpha$-$z$-R\'enyi divergences $D_{\alpha,z}(\psi\|\ffi)$ for normal positive
functionals $\psi,\ffi$ on a von Neumann algebra. We establish the sufficiency theorem for
$D_{\alpha,z}(\psi\|\ffi)$, saying that for $(\alpha,z)$ inside the DPI bounds, equality in the DPI under
a quantum channel implies the reversibility of the channel. Moreover, we show the monotonicity properties
of $D_{\alpha,z}(\psi\|\ffi)$ in the parameters $\alpha,z$ and their limits to the normalized relative entropy as
...
reversibility,
sufficiency,
%generalized $s$-number,
%modular automorphism group,
%dual action,
%left and right $L^p$-spaces,
%symmetric $L^p$-space,
...
The $\alpha$-$z$-R\'enyi divergences were {\color{blue}first introduced in \cite{jaksic2012entropic}, and
...
operators) $\rho$ and $\sigma$ on a finite dimensional {\color{blue}[I unify "finite dimensional" though I
don't know which is better, "finite dimensional" or "finite-dimensional"]} Hilbert space, the divergence
$D_{\alpha,z}(\rho\|\sigma)$ is defined by 
\[
D_{\alpha,z}(\rho\|\sigma)=\frac{1}{\alpha-1}\log
{\color{blue}\frac{\mathrm{Tr}\bigl(\rho^{\frac{\alpha}{2z}}\sigma^{\frac{1-\alpha}{z}}
\rho^{\frac{\alpha}{2z}}\bigr)^z}{\mathrm{Tr}\,\rho},}
\]
where $0<\alpha\ne 1$ and $z>0$. This family includes the {\color{blue}Umegaki} relative entropy in the
...
These two quantities have important applications, {\color{blue}in particular, in describing asymptotic error bounds in
...
positive and trace-preserving maps. For the {\color{blue}$\alpha$-$z$-R\'enyi} divergences, this means
that we require
\[
...
the limit $\alpha\searrow 1$, this proves {\color{blue}the same} property also for the relative entropy.
...
\max\{{\color{blue}\alpha/2},\alpha-1\}\le z\le \alpha.
\]
...
algebras form a suitable framework for such extensions, since {\color{blue}they} are general enough to
...
quasi-entropies were first introduced for states of von Neumann algebras {\color{blue}in}
...
introduced independently in \cite{berta2018renyi}  using {\color{blue}Araki-Masuda's} $L^p$-spaces and
in \cite{jencova2018renyi,jencova2021renyi} using {\color{blue}Kosaki's} $L^p$-spaces. A number of
...
an overview of both the Petz-type and {\color{blue}the} sandwiched R\'enyi divergences in the
von Neumann algebra setting.

Sufficiency {\color{blue}of} quantum channels with respect to a set of states was introduced by Petz
\cite{petz1986sufficient,petz1988sufficiency}. This definition was inspired by sufficient statistics 
{\color{blue}in} classical statistical models. The quantum counterpart in Petz's definition is that all
the states in the given set can be recovered by another channel. It is an important result
that this property is characterized by equality in the DPI for relative entropy, as well
...
for the standard R\'enyi divergence was proved in \cite{hiai2017different} in the interval $(0,1)\cup({\color{blue}1},2)$
and in \cite{jencova2017preservation, jencova2018renyi, jencova2021renyi} for the sandwiched version
with $\alpha\in(1/2,1)\cup(1,\infty)$. See also \cite{hiai2021quantum} for a summary of these {\color{blue}results}
{\color{blue}in} von Neumann algebras.  


The $\alpha$-$z$-R\'enyi divergences $D_{\alpha,z}$ in von Neumann algebras were {\color{blue}very recently}
introduced in \cite{kato2023aremark}, {\color{blue}based on the Haagerup $L^p$-spaces}, and a number of
...
and a variational expression were proved in the case $0<\alpha<1$. {\color{blue}In the infinite dimensional
$\mathcal{B}(\mathcal{H})$ case,[This seems better for quantum information people]}
$D_{\alpha,z}$ and their properties were studied also in \cite{mosonyi2023thestrong}.
...
{\color{blue}Section 2 of the present paper is a preliminary on $D_{\alpha,z}$. In Sec.~3} we first finish
the proof of the variational expression for $\alpha>1$ and prove the DPI, in the same bounds as in the
finite dimensional case. Similarly to {\color{blue}Zhang's approach} in \cite{zhang2020fromwyd}, we apply the
variational expressions to {\color{blue}do} this, but, not having the techniques of matrix analysis
at our disposal, we use their relation to the sandwiched R\'enyi divergences for the proof. This allows us
to show the DPI with respect to normal positive unital maps, similarly {\color{blue}to} the case $0<\alpha<1$
...
theorems for $D_{\alpha,z}$. {\color{blue}In Sec.~4 we next show the most significant result} that for the
...
conditional expectations {\color{blue}extended to the Haagerup $L^p$-spaces.}

We then turn to monotonicity in the parameters $z$ and $\alpha$. {\color{blue}In Sec.~5 we} first show that in the
...
on the interval $[\alpha/2,\infty)$ using {\color{blue}a} complex interpolation method
{\color{blue}based on Kosaki's $L^p$-spaces. In Sec.~6} we further prove monotonicity in the parameter
$\alpha$, which seems to be new even in the finite dimensional case, {\color{blue}using a real analysis method
...
{\color{blue}Appendices A--C are brief summaries on the Haagerup $L^p$-spaces, Haagerup's reduction theorem,
...
{We denote the unit of von Neumann algebras by $\1$.}
...
$\Me_*^+$ is identified with the positive cone $L^1(\Me)^+$, and $\states_*(\Me)$ with the set of elements
$h\in L^1(\Me)^+$ with $\Tr\,h=1$. {Let $\|a\|_p$ denote the Haagerup (quasi-)norm of $a\in L^p(\Me)$,
$0<p\le\infty$.} Precise definitions and further details on the spaces $L^p(\Me)$ can be
found in \cite[Chap.~9]{hiai2021lectures}, or in the notes \cite{terp1981lpspaces}. {A short summary
on the Haagerup $L^p$-spaces and} some technical results that will be used below can be found in
Appendix \ref{app:lp}.
...
\]
Moreover, {\color{blue}since $a_0^{-1}\le(a_1^{-1}+a_2^{-1})/2$}, using Lemma \ref{lemma:order1}
we have
\begin{align*}
...
\begin{align*}
&\<\nabla f(a),b-a\>\\
&\quad=\lim_{{\color{blue}s\searrow0}} {1\over s}[ f(sb+(1-s)a)-f(a)]\\
...
%defines a contraction {\color{red}between Kosaki's symmetric} $L^p$-spaces
%$L^p(\Ne,\rho\circ\gamma)\to L^p(\Me,\rho)$ (see \eqref{F-C.4} in Appendix \ref{Appen-Kosaki-Lp}),
...
In what follows, a (quantum) \emph{channel} is a normal 2-positive unital map $\gamma: \Ne\to \Me$.
{\color{blue}We use this term for the sake of convenience, while in most literature, a quantum channel is
assumed to be completely positive.}
...
{\color{blue}(also \cite{jencova2006sufficiency})} that sufficient channels can be characterized by
...
We now turn to the case $\alpha>1$. {\color{blue}Assume that $(\alpha,z)$ is within the DPI bounds,
...
{\color{blue}Let $(\alpha,z)$ be as assumed above.} Let $\gamma:\Ne\to\Me$ be a normal positive unital map. 
...
{\color{blue}Let $\alpha>1$ and $\max\{\alpha/2,\alpha-1\}\le z\le\alpha$, and assume further that
...
\]
Similarly {\color{blue}to} the proof of Theorem \ref{thm:suffle1} (note that we
...
\begin{theorem}[\mbox{\cite[Theorem 1(x)]{kato2023onrenyi}}]\label{T-4.1}
...
integrable on $(0,\infty)$, the Lebesgue convergence theorem yields \eqref{F-5.5}.
...
\]
for all $s\in(0,s_0]$. Therefore, it follows from \eqref{F-5.8} that
...
\]
one has, similarly to \eqref{F-5.8},
...
\]
for all $s>0$. Therefore, by \eqref{F-5.8} and \eqref{F-5.9} the monotone convergence theorem yields
\eqref{F-5.7}.
\end{proof}

\begin{lemma}\label{L-4.3}
Let $(\Me,\tau)$ and $\psi,\ffi$ be as in Lemma \ref{L-4.2}, and let $0<z\le z'$. Then
\[
\begin{cases}
...
\begin{proof}
The case $0<\alpha<1$ is in Theorem \ref{T-4.1} for general von Neumann algebras.
For the case $\alpha>1$, by Lemma \ref{L-4.2} it suffices to show that, for every $\eps>0$,
\[
...
When $(\Me,\tau)$ and $\psi,\ffi$ are as in Lemma \ref{L-4.2}, one can define, thanks to
Lemma \ref{L-4.3}, for any $\alpha\in(0,\infty)\setminus\{1\}$,
\begin{align}
...
\inf_{z>0}D_{\alpha,z}(\psi\|\ffi), & \text{$\alpha>1$}.\end{cases} \label{F-5.10}
\end{align}
...
Let $(\Me,\tau)$ and $\psi,\ffi$ be as in Lemma \ref{L-4.2}. Then for any $z>0$,
\[
\begin{cases}
...
Then by \eqref{F-5.11}, $F(\alpha)=\log Q_{\alpha,\infty}(\psi\|\ffi)$ for all $\alpha\in(0,\infty)\setminus\{1\}$,
and we compute
\begin{align*}
...
=D_1(\psi\|\ffi).
\]
Hence by \eqref{F-5.10} the assertion holds when $h_\psi,h_\ffi\in\Me^{++}$. Below we extend this to
...
$h_{\psi_n}^{\alpha/z}\searrow h_\psi^{\alpha/z}$ in the operator norm, we have by \eqref{F-5.6} and
...
(h_\ffi^{-1})^{\alpha-1\over2z}\Bigr)^z\,ds
=\lim_{n\to\infty}Q_{\alpha,z}(\psi_n\|\ffi). \label{F-5.12}
\end{align}
...
\eqref{F-5.6} and \cite[Lemma 3.4]{fack1986generalized} again
\begin{align*}
...
$h_{\psi_n}^{\alpha/z}\nearrow h_\psi^{\alpha/z}$ in the measure topology, one can argue as in \eqref{F-5.12}
with use of the monotone convergence theorem to see from (2) that the extension holds in this case too.

(4)\enspace
Finally, from (3) with Lemma \ref{L-4.2} and \cite[Corollary 2.8(3)]{hiai2021quantum} it follows that
...
Theorem \ref{T-4.1}, while we include it here to make the statement complete.

\begin{prop}\label{P-4.5}
...
\begin{proof}
(i) follows from Lemmas \ref{L-4.3} and \ref{L-4.4}.
...
\]
Hence \eqref{F-5.13} follows since it holds for $D_{\alpha,1}$ by
...
\]
Hence \eqref{F-5.14} follows since it holds for $D_{\alpha,\alpha}$ by
...
\ref{T-4.1}, for $0<\alpha<1$ this monotonicity holds in general von Neumann algebras. In the next
...
we will extend the  limits given in \eqref{F-5.13} and \eqref{F-5.14} in Sec.~5.3.


\subsection{The general von Neumann algebra case}\label{Sec-4.2}

{
From now on let $\Me$ be again a general von Neumann algebra. In the next proposition, we first
extend Proposition \ref{P-4.5}(i) to general von Neumann algebras, based on Haagerup's reduction
theorem \cite{haagerup2010areduction}, that is briefly explained in Appendix \ref{Appen-reduction} for
the convenience of the reader.

\begin{prop}\label{P-4.6}
...
$D_{\alpha,z}(\psi\|\ffi)\le D_1(\psi\|\ffi)$ by Proposition \ref{P-4.5}(i) and Lemma \ref{L-B.2} in
Appendix \ref{Appen-reduction}. Combining this and Theorem \ref{T-4.1} yields (i). Statement (ii) is
again a consequence of Proposition \ref{P-4.5}(i) and Lemma \ref{L-B.2}.
\end{proof}

For the proof of Proposition \ref{P-4.6}(ii) by use of Lemma \ref{L-B.2} (based on Theorem \ref{T-B.1})
it is inevitable to restrict the parameter $z$ to the DPI bounds when $\alpha>1$. However, in the next
theorem we show the $\alpha>1$ counterpart of Theorem \ref{T-4.1}, showing the monotonicity of
$D_{\alpha,z}$ in $z\ge\alpha/2$ when $\alpha>1$. Nevertheless, we note that the inequalities between
$D_{\alpha,z}$ and $D_1$ in (i) and (ii) of Proposition \ref{P-4.6} are not included in
Theorems \ref{T-4.1} and \ref{T-4.7}. The proof below is based on Kosaki's interpolation $L^p$-spaces
...
finite dimensional setting, as shown in \cite[Theorem 2.1]{hiai2024logmajorization}.
\end{remark}
...
%\textcolor{orange}{that is a semi-finite von Neumann algebra with the canonical semi-finite
%trace $\tau$, where $\sigma_t$ is the modular automorphism group associated with some faithful normal
%state (or weight). [Maybe skip this and just give a reference to Appendix \ref{app:lp}]}
...
\noindent
{\bf Proof of Theorem \ref{T-5.1}.}\enspace
We may assume that $s(\psi)\not\perp s(\ffi)$; otherwise, $Q_{\alpha,z}(\psi\|\ffi)=0$ for all
$\alpha\in(0,1)$. Then by Lemma \ref{L-5.4}, $Q_{\alpha,z}(\psi\|\ffi)\in(0,\infty)$ for all $\alpha\in(0,1)$,
and $\alpha\mapsto Q_{\alpha,z}(\psi\|\ffi)$ is continuous on $(0,1)$. Hence
$\alpha\mapsto D_{\alpha,z}(\psi\|\ffi)$ is continuous on $(0,1)$ too.

Let $\alpha_1,\alpha_2\in(0,1)$ and $z>0$. Let $(\cR,\tau)$ be as in the proof of Lemma \ref{L-5.4}. Consider
...
Theorem \ref{T-5.1} can also be proved, though restricted to $z>1/2$, by use of the complex
interpolation method based on Kosaki's interpolation $L^p$-spaces. It is worthwhile to sketch the
proof here. It suffices to prove (1) of Theorem \ref{T-5.1} as (2) is immediate from (1) as in the last
part of the above proof of Theorem \ref{T-5.1}. Assume that $z>1/2$ and let $p:=2z$ and
...
%Assume that $z\ge1$, and put $h_0:=h_\psi^{1/2}h_\ffi^{1/2}\in L^1(\Me)$. For each $\alpha\in(0,1)$ put
...
%Q_{\alpha,z}(\psi\|\ffi)\le Q_{\alpha_1,z}(\psi\|\ffi)^{1-\theta}Q_{\alpha_2,z}(\psi\|\ffi)^\theta.
%\end{align}
%From the complex interpolation space in \eqref{F-6.7} and the reiteration theorem \cite{cwikel1978complex},
%we have
%\begin{align}\label{F-6.10}
%L^p_\eta=C_\theta(L^p_{\eta_1},L^p_{\eta_2}).
%\end{align}
%Since $h_0\in L^p_{\eta_1}\cap L^p_{\eta_2}$ as shown above, it follows that
...
%From \eqref{F-6.11} and \eqref{F-6.9} (for $\eta,\eta_1,\eta_2$) we have \eqref{F-6.8}, so that (1) has been
...
%Let us denote $\Sigma=\Sigma(L^p_L,L^p_R):=L^p_L+L^p_R$, and let
%\textcolor{orange}{$\tilde \cF(L^p_L,L^p_R)$} be
...
%We have $f\in\textcolor{orange}{\tilde \cF(L^p_L,L^p_R)}$. Moreover, for each $\eta\in(0,1)$ and $t\in\bR$,
%$f(\eta+it)\in L^p_\eta$ and
%\[
...
%defines a strongly continuous one-parameter group of isometries on $L^p(\Me)$ for every $p\in[1,\infty)$.
%This implies properties (ii) and (iii) in the definition of $\textcolor{orange}{\tilde
%\cF(L^p_L,L^p_R)}$. Furthermore, for $\eta\in(0,1)$
%we see that $f(\eta+it)\in L^p_\eta$ and
%\[
...
%\]
%Since $L^p_\eta$ for each $\eta\in(0,1)$ is continuously embedded in $\Sigma$, this implies that $f$ is
...
%$\eta\in(0,1)$, $f$ is also bounded on $S$. Note that as a function with values in $L^1(\Me)$, $f$ is bounded,
...
%\]
%The integrals are in $L^1(\Me)$, but since $t\mapsto f(it)\in L^p_L$ and $t\mapsto f(1+it)\in\L^p_R$ are
%continuous and bounded in the respective norms, we see that the integrals also exist in $\Sigma$. Hence
%the above equality holds in $\Sigma$ since $\Sigma$ is continuously embedded in $L^1(\Me)$. This shows
...
%analytic in $L^1(\Me)$, the expression is equal to $f(w)$, which shows that $f$ is analytic in $\Sigma$ in
%the interior of $S$.
%\end{proof}
%
%\noindent
...
%$f$ given in \eqref{F-6.10} define $f_1(w):=f((1-w)\eta_1+w\eta_2)$, which belongs to the set
%\textcolor{orange}{$\tilde \cF(L^p_{\eta_1},L^p_{\eta_2})$} %(see \cite[Definition 1.1]{kosaki1984applications}) 
%by Lemma \ref{L-5.5} and \eqref{F-C.6}. Since $L^p_\eta=C_\theta(L^p_{\eta_1},L^p_{\eta_2})$ by the
...
%%Furthermore, since $f(it)\in L^p_L$ and $\|f(it)\|_{L^p_L}^p=\psi(\1)$, (2) is seen as in the above case.
%\qed
...
$z>1/2$, based on the complex interpolation in a similar way {\color{blue}to arguments} in
Remark \ref{remark:2nd-proof}.

\begin{theorem}\label{T-5.6}
...
$\alpha\searrow1$, extending the limits in \eqref{F-5.1} and \eqref{F-5.2}.
...
below and \eqref{F-5.1} for $D_{\alpha,1}$. On the other hand, for the case $z\in[1,\infty)$ the result
follows from Proposition \ref{P-4.6}(i) as \eqref{F-5.13} does from Proposition \ref{P-4.5}(i).
\end{proof}
...
from Proposition \ref{P-4.6}(i). For the first inequality, noting that $\beta\in(0,1)$ by assumption and 
...
$\ffi$ is faithful. We utilize the function $f$ on $S$ given in \eqref{F-5.15}, whose values are in $L^{2z}_L$
as seen from the proof of Theorem \ref{T-4.7}. Since $f$ is analytic in a neighborhood of $1/\alpha$, we have
the expansion
\[
...
\]
Furthermore, as in the proof of Theorem \ref{T-4.7}, we have
...
thank Yoshimichi Ueda for his arranging the meeting, and are also grateful for Ueda and Kato for discussions.

{\color{blue}[It might be goof for you to thank Francesco Buscemi for his inviting you. Probably you have grants.]}
...
{\color{blue}Let $\Me$ be a von Neumann algebra on a Hilbert space $\mathcal{H}$. Let $\omega$ be
...
space $\mathcal{H}\otimes L^2(\bR)$ with the canonical trace $\tau$. [Here I refer to the space
$\mathcal{H}\otimes L^2(\bR)$ since $\mathrm{id}_{L^2(\bR)}$ appears below.]}
...
$\omega|_\Ne$ and we may identify $\cS:=\Ne \rtimes_{\sigma^{\omega_\Ne}}\bR$
...
\end{equation}
see \cite[Prop.~2.3]{junge2003noncommutative}.
...
we give the next lemma, which is used in Sec.~\ref{Sec-4.2}.
...
=\lim_{n\to\infty}D_1(\hat\psi|_{\Me_n}\|\hat\ffi|_{\Me_n})\quad\mbox{increasingly}. \label{F-5.17}
\end{align}
\end{lemma}
...
the first equality in \eqref{F-5.16}. By Theorem \ref{T-B.1} we can apply the martingale convergence
in Theorem \ref{thm:martingale} to obtain the latter equality in \eqref{F-5.16} with increasing convergence.
The assertion of $D_1$ in \eqref{F-5.17} is included in \cite[Proposition 2.2]{fawzi2023asymptotic},
