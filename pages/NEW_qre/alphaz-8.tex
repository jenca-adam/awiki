...
{\color{blue}We denote the unit of von Neumann algebras by $\1$.}
...
{\color{blue}note that $L^\infty(\Me)=\Me$ [''equality'' holds rather than isomorphism], and use the identification
$\Me_*\ni \psi \leftrightarrow h_\psi\in L^1(\Me)$ (an order isomorphism)},
so that the $\Tr$-functional on $L^1(\Me)$ defined by $\Tr\,h_\psi:=\psi(\1)$ for $\psi\in \Me_*$. It this way,
$\Me_*^+$ is identified with the positive cone $L^1(\Me)^+$, and $\states_*(\Me)$ with the set of elements
$h\in L^1(\Me)^+$ with $\Tr\,h=1$. {\color{blue}Let $\|a\|_p$ denote the Haagerup (quasi-)norm of $a\in L^p(\Me)$,
...
$L^1(\Me)^{(1)}$, where $\Tr_j$ is the $\Tr$-functional on $\cR_j$. (For more details on the isomorphism
...
\end{remark}
[It seems to me that this remark has to be given from the theoretical point of view.]}
...
Let $\alpha>1$ and $\psi,\varphi\in \Me_*^+$, {\color{blue}$\psi\ne0$.} Then $Q_{\alpha,z}(\psi\|\varphi)<\infty$
...
{\color{blue}(Note that this follows also from the order relations in \cite[Theorem 2(iii)]{kato2023onrenyi}).}
...
\eqref{F-1.2}, {\color{blue}so that
...
\]
Here, $\psi(\1-e_i)^\alpha\ffi(\1-e_i)^{1-\alpha}$ is defined with the {\color{blue}usual} conventions
\[
...
{\color{blue}Moreover, by Lemma \ref{lemma:dpi}(ii) and \eqref{eq:infimum} note that
\[
...
inequality (?)]} {\color{blue}[Yes, I made a mistake, sorry. I have given \eqref{eq:infimum} in more detail.
Please check.]}
...
2z}\in L^{2z}(\mathcal E(\Me))^+$. By the assumption $2z>1$ note that we may use the
...
polar decomposition in $L^{2z}(\Me)$; then we have 
\[
...
In the rest of the paper we will discuss similar monotonicity properties {\color{blue}of $D_{\alpha,z}$ in
...
{\color{blue}The next theorem in \cite{kato2023onrenyi} shows the monotonicity of $D_{\alpha,z}$ in the
...
\end{theorem}
[It seems better to first write down this to clarify the situation.]}
...
{\color{blue}In the rest of this subsection,} we assume that $(\Me,\tau)$ is a finite von Neumann algebra
...
{\color{blue}Let $(\Me,\tau)$ be as mentioned above.}
...
so that $e_n\nearrow\1$. {\color{blue}Hence by Proposition \ref{prop:mart-proj} one has
...
{\color{blue}Note that the first inequality in (i) of the next proposition is a particular case of
...
case \cite{lin2015investigating,mosonyi2023somecontinuity}. In the next subsection, when $\alpha>1$
(see Theorem \ref{T-4.1} when $0<\alpha<1$), we will extend it to general von Neumann algebras
under a restriction on $z$ by using the complex interpolation method. {\color{blue}Furthermore,
in Sec.~5.3 we will obtain similar limits to \eqref{F-4.13} and \eqref{F-4.14} in general von Neumann
algebras by the complex interpolation method.}
...
\qquad t\in\bR,
\end{align*}
where the last equality follows from \cite[Lemmas 10.1]{kosaki1984applications}. Furthermore,
we observe that $f$ is a bounded continuous function on $S$ into $L^1(\Me)$
(see \cite[Lemmas 10.2]{kosaki1984applications}) and it is analytic in the interior
...
\cite{cwikel1978complex} {\color{blue}for the best result on that}), $C_\theta=L^{2z/\theta}_L$
...
$\cR:=\Me\rtimes_{\sigma^\omega}\bR$ (see Appendix \ref{app:lp}). {\color{blue}[Is this OK?]}
...
should settle on lower or upper index, I don't know which is better.]} {\color{blue}[It seems that upper index
$L^p$ is a bit more familiar, so I have fixed $L^p$ in all the places at the moment. Please check.]}
...
interpolation method based on Kosaki's interpolation $L_p$-spaces. It is worthwhile to sketch the
...
based on the complex interpolation {\color{blue}as discussed in Remark \ref{remark:2nd-proof}.}
...
{\color{blue}[It seems better to give this lemma here in Appendix B.]}
