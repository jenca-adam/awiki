...
A divergence $D$ should satisfy the \structure{postulates}:
\vskip 3mm
\begin{itemize}
\item \structure{invariance} under permutations: $D(\pi(p)\|\pi(q))=D(p\|q)$
\item \structure{continuity}
\item \structure{additivity}: $D(p_1\otimes p_1\|q_1\otimes q_2)=D(p_1\|q_1)+D(p_2\|q_2)$
\item \structure{generalized mean}: for a continuous, strictly
increasing real function $g$ %:\mathbb R\to \mathbb R$
\[
D(p_1\oplus p_2\|q_1\oplus
q_2)=g^{-1}\left(\frac{g(D(p_1\|q_1)+g(D(p_2\|q_2))}{2}\right)
\]
\item \structure{order}: if $p$ is not normalized
\[
p_i\le q_i, \ \forall i \implies D(p\|q)\ge 0,\qquad p_i\ge q_i,\ \forall i\implies
D(p\|q)\le 0
\]
\item \structure{normalization}: $D(\{1\}\|\{1/2\})=1$
\end{itemize}
...
satisfying the \structure{R\'enyi postulates}:
...
\item For $\alpha=1$, we get the  \structure{Kullback-Leibler divergence} (relative entropy,
$I$-divergence)
...
\begin{itemize}
\item A \structure{test statistic}: $T$
\vskip 3mm
\item Two kinds of \structure{error probabilities}:
\[
...
\bigskip
\structure{Tradeoff} between $R$ and $R'$:
