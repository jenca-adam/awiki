...
\item \structure{additivity}: $D(p_1\otimes p_2|q_1\otimes q_2)=D(p_1\|q_1)+D(p_2\|q_2)$
...
\]
\item \structure{order relations}:
\[
...
%\item Important \structure{operational interpretations}
\end{itemize}
...
\bigskip
\structure{Trade-off} between $R$ and $R'$:
...
\bigskip
\structure{Kullback-Leibler-Csisz\'ar Theorem}: If $\supp(p)\subseteq \supp(q)$, $\alpha>0$
\[
...
\vskip 5mm
\item In  quantum case: \structure{no unique solution}
...
\vskip 2mm
\item \structure{operational interpretations}:  quantum communication,
...
\]
\item \structure{Quantum sandwiched case}: $\sigma$-weighted $L_p$-norm {\small(AJ, 2018)}
\[
