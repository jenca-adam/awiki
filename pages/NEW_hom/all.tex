...
Let $X_1,\dots,X_n$ be first order objects in $\Af$, so that $X_i=(V_i, \{\tilde
a_i\}^\sim)$. For each $i\in [n]$ let us pick some $a_i\in \{\tilde a_i\}^\sim$. 
Let us define
\[
L_{i,0}:=\mathbb R \{a_i\},\qquad L_{i,1}:= \{\tilde a_i\}^\perp,\qquad i\in [n].
\]
Then $L_{i,0}\cap L_{i,1}=\{0\}$ and $V_i=L_{i,0}\oplus L_{i,1}$. Put
\[
L_s:=L_{1,s_1}\otimes\dots \otimes L_{n,s_n},\qquad s\in \{0,1\}^n,
\]
then $L_s$, $s\in \{0,1\}^n$ is an independent decomposition of $V:=V_1\otimes \dots
\otimes V_n$. Similarly, put
\[
\tilde L_{i,0}:=\mathbb R \{\tilde a_i\},\qquad \tilde L_{i,1}:= \{a_i\}^\perp,\qquad i\in
[n],
\]
then $V^*_i=\tilde L_{i,0}\oplus \tilde L_{i,1}$ and we obtain an independent
decomposition of $V^*=V_1^*\otimes\dots \otimes  V^*_n$ as 
\[
\tilde L_s:=\tilde L_{1,s_1}\otimes\dots \otimes \tilde L_{n,s_n},\qquad s\in
\{0,1\}^n.
\]
Note also that 
\begin{equation}\label{eq:complement}
L_{i,u}^\perp=\tilde L_{i,\bar u},\qquad u\in \{0,1\},\ i\in [n].
\end{equation}
...
\]
and the dual affine subspace  $\tilde A_f$ is determined by 
\[
S_{\tilde A_f}= \bigoplus_{s\in \{0,1\}^n} f^*(s)\tilde L_s.
\]
...
\{\tilde a\}^\sim$, we see that $0\ne A_f$. It follows that $A_f$ is proper and $\tilde
...
\end{enumerate}
In particular, the map $f\mapsto A_f$ is injective, this also follows from the fact that
$f\mapsto S_f$ is injective, since $L_s$ form an independent decomposition. Moreover, the set $\{A_f,\ f\in
...
pullbacks and pushouts as lattice operations.
...
order objects $Y_1,\dots, Y_n$, we will write $Y\sim\{Y_1,\dots,Y_n\}$ in this case. By compactness of $\FV$, we may assume
 (relabeling the objects if necessary) that the vector space of $Y$ has the form
\[
V_Y=V:=V_{1}\otimes \dots\otimes V_{n},
\]
where  $V_i$ is either $V_{X_i}$ or $V_{X_i}^*$, according to whether $Y_i$ was subjected
...
the set of inputs. The reason for this terminology will become clear later. Let now
$X_1,\dots, X_n$ be objects such that $X_i=Y_i=(V_i,\{\tilde a_i:=\tilde a_{Y_i}\}^\sim)$ if $i\in O$ and $X_i=(V_i, \{\tilde
a_i\}^\sim)$ for $i\in I$, where in this case $\tilde a_i$ is an arbitrary but fixed element in
$A_{Y_i}$. (Notice that since $i\in I$, $V_i=V_i^*$.)
For the set of first order objects $X_1,\dots, X_n$ and $f\in \Fe_n$, let  $A_f$ and $X_f$
be defined as above. 

\begin{prop}\label{prop:boolean}
With the above definitions, there is a unique function $f\in \Fe_n$ such that $Y= X_f$.
...
all $m<n$. By construction,  up to a permutation of the spaces, $Y$ is either the tensor
...
\]
 ...
 $O_{Z_1}\cup O_{Z_2}=O_Y$, and similarly for $I$, so that the correcponding objects $X_i$
 remain the same. By the induction 
...
it also  holds for $Y^*$. So assume that $Y=X_f=(V, A_f)$ for some $f\in \Fe_n$.
Then $Y^*=(V^*,\tilde A_f)$, and by Lemma \ref{lemma:Xf}, we see that $Y^*$ has the
required form, up to replacing  the spaces $L_s$ by $\tilde L_s$, which corresponds to
swapping the inputs and outputs. Since this is precisely what happens after taking the
duals, the statement is proven.
 
\end{proof}

It is plausible (and we will show an example later) that not all objects of the form $X_f$
are higher order objects.  We will specify  the set of functions that corresponds to such
...
(1\otimes 1^*)^*\},
\]
etc.  Elements of $\Te_n$ will be called type functions. Similarly as for the higher order
...
$f\in \Te_n$, $O=O_f$, $I=I_f$. Then $Y=X_f$ is a higher order objects with $O_Y=O$ and  $Y\sim
\{Y_1,\dots, Y_n\}$, where $Y_i=X_i$ for $i\in O$ and $Y_i=(V_{i}^*, \{a_i\}^\sim)$ for
$i\in I$.  
...
\]
so that $X_f=(V_1,\{a_1\})=(V_1^*,\{a_1\}^\sim)^*=Y_1^*$. Assume next that the statement
...
for $f^*$. Using Lemma \ref{lemma:Xf} and its proof, we can see that $X_{f^*}$ is the dual
of the object $\tilde X_f$, constructed by replacing $L_{i,u}$ by $\tilde L_{i,u}$, for
each  $i$ and $u\in \{0,1\}$, which is the same as replacing each $X_i$ by the object
$\tilde X_i=(V_i^*,\{a_i\}^\sim)$ (and then choosing $\tilde a_i$ as the special fixed
element of $A_{\tilde X_i}$). Hmmm.
\[
S_f^*= (\big
\]
