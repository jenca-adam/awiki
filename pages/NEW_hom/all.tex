...
\title{On the structure of quantum higher order maps}
...
In Appendix we describe the category $\Af$, whose objects  are of the form $X=(V_X,A_X)$, where $V_X$ is a finite dimensional
real vector space and $A_X\subseteq V_X$ a proper affine subspace, and morphisms $X\xrightarrow{f} Y$ are linear maps $f:V_X\to V_Y$  such that
$f(A_X)\subseteq A_Y$. It is also shown that we can introduce a tensor product $\otimes$
