...
%\item[(ii)]  $f=f_1\otimes f_2$ for some type functions
%$f_1$ and $f_2$ if and only if the condition in (i) is not fulfilled. In that case, define
%a relation $\sim$ on elements of $\Pe_f^0$ as $S\sim T$ if $L_S=L_T$ and order the
%equivalence classes as $[S]_\sim \le [T]_\sim$ if $S'\le T'$ for some $S'\sim S$ and
%$T'\sim T$. Then $\Pe=\Pe_f^0|_\sim$ becomes a labelled poset, with labels
%$L_{[S]_\sim}=L_S$ and $\mathrm{Min}(\Pe)=\mathrm{Min}(\Pe_f^0)$
%
%\end{enumerate}
%
%
