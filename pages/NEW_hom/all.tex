...
standard if both $A_X$ and $\tilde A_X$ contain a positive multiple of the identity matrix
$E_n$\footnote{We use the notation $E_n$, and not $I_n$, to avoid the slight chance that
it might be confused with the unit object.}. 
...
standard, then so are $X^*$ and $X\otimes Y$.
...
R^N$ and the positive cone by $\mathbb R_+^N$. Classical objects are standard if both
...
\begin{exm}[Higher order quantum maps] \label{exm:quantum_maps} The basic example of a standard quantum object
describes the set of quantum states. Let  
\[
\mathcal A_n:=\{T\in M_n^h, \ \Tr[T]=1\}.
\]
Then $\Se_n:=(M_n^h,\mathcal A_n)$ is an object in $\Af$, and it is a standard quantum object, since we have 
 $E_n\in \tilde {\mathcal A}_n=\{E_n\}$ and $\frac 1n E_n\in
\mathcal A_n$. 
The set $\mathcal A_n\cap M_n^+$ is the set of quantum states. By Proposition
\ref{prop:ihom_morphisms}, $\Ce_{m,n}:=[\Se_m,\Se_n]$ is a standard quantum object as well, such
...
standard quantum object, corresponding to the set of Choi matrices of quantum
superchannels, or 2-combs, etc. Note that in this way we always obtain standard quantum
...
\begin{exm}[Partially classical maps]\label{exm:quantum_classical}  We may similarly define the basic standard
classical object as 
\[
\Pe_N:=(\mathbb R^N, \{x,\ \sum_i x_i=1\}).
\]
In this case, $\mathcal A_{\Pe_N}\cap \mathbb R^N_+$ is the probability simplex. We then
obtain further  useful objects by combining with the standard quantum objects. For example, it can be
...
\]
for first order objects $X_1,X_2,\dots,X_{2n}$. For standard quantum objects, we see that
an $n$-comb describes the sets of $n$-combs introduced in {}. (We slightly  abuse
...
It is clear that for a first order quantum object we necessarily have  $\tilde a_X\in
int(M_n^+)$, similarly for classical first order objects. In this case, we will also
assume that $a_X\in int(M_n^+)$. A standard first order quantum
...
\]
where $\rho$ denotes the symmetry given by the transposition in $\permut_2$,  so that $p^*_T=1-p_T+p_{[n]}$ corresponds to general channels with output indices $T$.
...
Combining this with Proposition \ref{prop:objects_to_f}, we get the following result
(cf. cite). 
...
$\hat f_T\ne 0$. We will show that any type function $f\in \Te_n$ is fully determined by $\Pe_f$.  
...
$\rho_f=\rho_{f_1}+\rho_{f_2}$ ([Stanley]), This
...
\in \Fe_{n_2}$,  is a function such that  $\Pe_{f'}$ contains both $\emptyset$ and $[n_2]$ and  is
...
here $m_i=n_{i+1}-n_2$. By Proposition \ref{prop:objects_to_f}, we see that 
\[
...
\Te_{n_k}$, finite index sets $A$, $B$ and permutations $\pi_{a,b}\in
...
\[
\biggl(\bigvee_{a\in A}\bigwedge_{b\in B}\beta_1^{a,b}\biggr)\otimes \biggl(\bigvee_{c\in
C}\bigwedge_{d\in D}\beta_2^{c,d}\biggr)=\bigvee_{a,c}\bigwedge_{b,d}
...
get
\begin{align*}
&\biggl(\bigwedge_{b\in B}\bigvee_{a\in A}\beta_1^{a,b}\biggr)\otimes \biggl(\bigwedge_{d\in D}\bigvee_{c\in
C}\beta_2^{c,d}\biggr)\\
&\quad =\left[\biggl(\bigwedge_{b\in B}\bigvee_{a\in A}\beta_1^{a,b}\biggr)\vtl \biggl(\bigwedge_{d\in D}\bigvee_{c\in
C}\beta_2^{c,d}\biggr)\right]\wedge \left[\biggl(\bigwedge_{d\in D}\bigvee_{c\in
C}\beta_2^{c,d}\biggr)\vtl\biggl(\bigwedge_{b\in B}\bigvee_{a\in A}\beta_1^{a,b}\biggr)
\right]\\
&= \left(\bigwedge_{b,d}\bigvee_{a,c} \beta_1^{a,b}\vtl \beta_2^{c,d}\right) \wedge
\left(\bigwedge_{b,d}\bigvee_{a,c} \beta_2^{c,d}\vtl \beta_1^{a,b}\right).
\end{align*}
