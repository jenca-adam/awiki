...
\[
O_f^F:=[n]\setminus
\cup_{S\in \Pe_f}L_S, \qquad I_f^F:=\cap_{S\in \Pe_f} L_S.
\]
...
\begin{lemma}\label{lemma:pf0_smallest} Let $f\in \Te_n$ be such that either $\Pe_f$ or
$\Pe_{f^*}$ has a smallest element with a nonempty label set. 
Then either $f$ is a chain type, or there is some chain type $\beta\in
\Te_k$ and some $h\in \Te_{n-k}$ such that $h$ nor $h^*$ has a smallest element with
nonempty label set and $f=\beta\vtl
h$. 

\end{lemma}


\begin{proof} Assume  $\{S\}=\mathrm{Min}(\Pe_f^0)=\mathrm{Min}(P_f)$, then
$S=L_S=\cap\Pe_f=I^F_f$.  If $S\ne \emptyset$, then up to a permutation, $f=p_k\otimes
h=p_k\vtl h$,
